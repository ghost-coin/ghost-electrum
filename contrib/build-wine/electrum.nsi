...
;Variables
  ...
  !define PRODUCT_NAME "Ghost Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/bleach86/electrum"
  !define PRODUCT_PUBLISHER "Ghost Core Developers"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/ghostelectrum-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\ghostelectrum-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\ghostelectrum-${PRODUCT_VERSION}.exe" "" "$INSTDIR\ghostelectrum-${PRODUCT_VERSION}.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\ghostelectrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\ghostelectrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
